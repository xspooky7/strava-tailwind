...
export const loadScript = async () => {
  ...
  for (const starSeg of starredSegments) {
    const cachedSeg = dbSegments.find(
      (dbSeg: DetailedSegment) => dbSeg.id === starSeg.id
    )
  ...
  const mergedSegmentsForUpdate: DetailedSegment[] =
    cachedSegments.concat(newSegments)
  const updateStatus = await setDatabase(
    "tailwind/segments",
    mergedSegmentsForUpdate
  )
