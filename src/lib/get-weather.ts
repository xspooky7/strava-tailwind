 ...
 * @returns {Object} - An object holding respective weather data for the segment
 */

export const getWeather = async (path: Line[]) => {
	let meteoRequests = 0
	...
	for (let i = 0; i < path.length; i++) {
		...
		if (angleDifference <= 45) tailAbs += line.distance
