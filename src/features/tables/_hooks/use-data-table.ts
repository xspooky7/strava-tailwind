...
interface UseDataTableProps<TData>
   ...
   * @default []
   * @type { label: string, value: keyof TData, placeholder?: string, options?: { label: string, value: string }[] }[]
   * @example
   * ```ts
   ...
   * ```
   */
  filterFields?: TableFilterField<TData>[]
...
export function useDataTable<TData>({
  ...
  const [perPage, setPerPage] = useQueryState(
    "perPage",
    parseAsInteger.withOptions(queryStateOptions).withDefault(initialState?.pagination?.pageSize ?? 30)
  )
  const [sort, setSort] = useQueryState(
    "sort",
    parseAsString
      .withOptions(queryStateOptions)
      .withDefault(`${initialState?.sorting?.[0]?.id}.${initialState?.sorting?.[0]?.desc ? "desc" : "asc"}`)
  )
  ...
  const filterParsers = React.useMemo(() => {
    return filterFields.reduce<Record<string, Parser<string> | Parser<string[]>>>((acc, field) => {
      if (field.options) {
        // Faceted filter
        acc[field.value as string] = parseAsArrayOf(parseAsString, ".").withOptions(queryStateOptions)
      } else {
        // Search filter
        acc[field.value as string] = parseAsString.withOptions(queryStateOptions)
      }
      return acc
    }, {})
